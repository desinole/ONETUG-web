...
namespace Core
{
    public class MeetupGroup
    {
        ...
        private string _groupInfoUrl = string.Format("https://api.meetup.com/2/groups?&sign=true&photo-host=public&group_urlname=ONETUG&page=20&key={0}", ConfigurationManager.AppSettings["MeetupAPI"]);

        public MeetupGroup()
        {
            string jsonResponse = ONETUGRequest.GetResponse(_groupInfoUrl);
            dynamic d = JObject.Parse(jsonResponse);
            this.GroupUrl = "http://www.meetup.com/ONETUG/";
            ...
            this.Name = "ONETUG - Orlando .NET User Group";
